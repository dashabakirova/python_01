...
class MyWindow:
    ...
    def calc_prop(self, current_object):
        ...
        if len(source) > 0:
            ...
            for params in obj_['parametrs']:
                ...
                index += 1
            if current_object['id'] == self.browsed_object['id']:
                self.lb_source_q[index].config(text=f'Сумма долей = ({source_q})')
        else:
            source_q = 0

        sum_q = 0
        t = 0
        # Если работаем с текущим НПС, то значения берём из полей ввода, иначе - ис списка параметров
        if current_object['id'] == self.browsed_object['id']:
    ...
    def update_win_param(self):
        ...
        # Определяем первый индекс q родительской НПС и выводим их на форму
        ...
        if len(source) > 0:
            ...
            for params in obj_['parametrs']:
                ...
                s += 15
                self.heith += 15
            self.lb_source_q.append(packObject(Label, self.param_win, 110, 80 + s, {"text": f'Сумма долей = ({sum})'}))
            s += 15
